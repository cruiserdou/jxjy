...
@Controller
@RequestMapping("/print_train_roster_info")
public class PrintExamRoster {
    ...
    DataShop getShopInJSON(
        ...
        try{
            ...
            String sql = "select RANK() OVER(ORDER BY id) as 序号,name as 姓名,sex as 性别,card as 身份证号,address as  工作单位或家庭住址," +
                    "lictype as 准驾车型,licdt as 驾照初领日期,licmd as 资格类别 " +
                    " from work.trainer where status in ('终审','同意考试')  ";
...
//            font0.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);
            ...
            ce.setCellValue(new HSSFRichTextString("武威市道路运输行业从业人员培训花名册"));
