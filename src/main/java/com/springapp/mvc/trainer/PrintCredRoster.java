...
@Controller
@RequestMapping("/print_cred_roster_info")
public class PrintCredRoster {
...
//        String fileName= "ZhiZheng_roster.xls";
        try{
            ...
            String sql = "select RANK() OVER(ORDER BY id) as 序号,name as 姓名,sex as 性别,card as 身份证号,address as  工作单位或家庭住址," +
                    "lictype as 准驾车型,licdt as 驾照初领日期,licmd as 资格类别,'同意' as 是否同意考试,scores as 成绩,'' as 备注 " +
...
//            font0.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);
            ...
            ce.setCellValue(new HSSFRichTextString("武威市道路运输行业从业人员制证、发证登记册"));
            ...
            if(service != null){
                try {
                    ...
                    FileInputStream fis = new FileInputStream(projectPath_target+ "/" +fileName);
