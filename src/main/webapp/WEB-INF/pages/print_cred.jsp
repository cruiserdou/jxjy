...
<div id="wrap">
    ...
    <table   style=" width: 100%;margin:auto border:1px solid red;border-collapse:collapse;">
        ...
        <td colspan="1" width="6%">是否同意考试</td>
        <td colspan="1" width="5%">成绩</td>
        <td colspan="1" width="6%">备注</td>
        </tr>
        <%
            sqlBean db = new sqlBean();
            String sql = "select name,sex,card,address,lictype,licdt,licmd,status,scores,remark from work.trainer where status='同意考试'";
            ResultSet rs = db.executeQuery(sql);
            int i = 0;
            while(rs.next()){ ++i;%>
        <tr>
            <td colspan="1" width="5%"><%=i%></td>
            <td colspan="1" width="7%"><%=rs.getString("name")%></td>
            ...
            <td colspan="1" width="6%">同意</td>
            <td colspan="1" width="5%"><%=rs.getString("scores")%></td>
            <td colspan="1" width="6%"></td>
        </tr>
        <%} %>
    </table>
